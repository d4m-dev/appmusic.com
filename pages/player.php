...
$stmt = $conn->prepare("SELECT songs.*, 
                        ...
                        (SELECT AVG(rating) FROM ratings WHERE ratings.song_id = songs.id) AS avg_rating
                        FROM songs 
                        WHERE songs.id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['like']) && isset($_SESSION['user'])) {
    $user_id = $_SESSION['user']['id'];
    $song_id = $_POST['song_id'];
    $action = $_POST['like']; // 'like' hoặc 'dislike'

    // Xử lý Like hoặc Dislike
    if ($action === 'like') {
...
<body class="font-sans bg-white box-border">
  ...
  <div class="bg-gradient-to-r from-[#fdfbfb] to-[#ebedee] p-8 rounded-2xl shadow-lg w-[80vh] max-w-[380px] text-center backdrop-blur-sm mx-auto mb-10">
    <div class="song-info">
      ...
      <p class="my-1 mx-0 text-sm text-gray-600"><strong>Xếp hạng:</strong> <?php echo number_format($song['avg_rating'], 1); ?></p>
    </div>
